...
import { GET_RECIPE, GET_USER_RECIPE } from "../../api/graphql/queries";
...
const Recipe = ({ params }: { params: { recipeId: string } }) => {
  ...
  // }
  return (
    <div className="pt-20 whitespace-pre-line">
      <div className="px-60 bg-lightPink">
        ...
        <div className=" mt-5">
          ...
          <ol className="list-decimal whitespace-pre-line">
            {data.recipe.description.map(
              (des: { _id: string; value: string }) => {
                return (
      ...
      <br />
      <MoreRecipes userId={data.recipe.user._id} recipeId={data.recipe._id}/>
    </div>
  );
};
